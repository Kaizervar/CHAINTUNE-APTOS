// /// With the Music DAO, token holders can
// /// - be able to create a DAO and connect it to their existing Music project
...
//     use chain_tune::bucket_table::BucketTable;
//     use chain_tune::bucket_table;
//     use chain_tune::music_dao_events::{Self, emit_create_dao_event};
//     use std::bcs;
//     use std::error;
...
//         /// The Music token Collection that is used to govern the DAO.
//         governance_token: GovernanceToken,
//         /// The voting duration in secs.
//         voting_duration: u64,
//         /// Minimum required voting power (number of Music tokens) an account must have to create a proposal.
...
//         /// The creator address of this Music token collection
//         creator: address,
//         /// The collection name
//         collection: String,
//     }
...
//     #[view]
//     /// Get the proposal
//     public fun get_proposal(proposal_id: u64, music_dao: address): Proposal acquires Proposals {
//         assert!(exists<Proposals>(music_dao), error::not_found(EPRPOSALS_NOT_EXIST_AT_ADDRESS));
//         let proposals = &borrow_global<Proposals>(music_dao).proposals;
...
//     public fun get_proposal_resolution(proposal_id: u64, music_dao: address): u8 acquires Proposals {
//         let proposal = get_proposal(proposal_id, music_dao);
//         proposal.resolution
//     }

//     #[view]
//     /// Unpack the DAO fields
//     public fun unpack_dao(music_dao: address): (String, u64, address, String, u64, u64, u64, address, Option<address>) acquires DAO {
//         let dao = borrow_global<DAO>(music_dao);
//         (
//             dao.name,
...
//         // initalize token store and opt-in direct Music token transfer for easy of operation
...
//         account: &signer,
//         music_dao: address,// resource account address of the music dao
//         name: String,// name of the proposal
//         description: String,// description of the proposal
//         function_names: vector<String>,// 3 types of functions are supported: (1) "no_op", (2) "transfer_fund" and (3) "offer_nft"
//         // need to implement function names according to the dao requirements: (1) "threshold amount of tokens to be a verified artist", (2) "downloads", 
...
//         let dao = borrow_global_mut<DAO>(music_dao);
...
//         let proposal_store = borrow_global_mut<Proposals>(music_dao);
...
//         music_dao_events::emit_create_proposal_event(
//             admin_addr,
//             music_dao,
//             proposal_id,
//             name,
...
//             error::invalid_argument(ETOKEN_NAME_COUNT_NOT_MATCH_PROPERTY_VERSION_COUNT)
//         );
//         assert!(exists<DAO>(music_dao), error::not_found(EDAO_NOT_EXIST));
//         let dao = borrow_global_mut<DAO>(music_dao);
//         let gtoken = &dao.governance_token;
//         let proposals = borrow_global<Proposals>(music_dao);
...
//         let prop_stats = borrow_global_mut<ProposalVotingStatistics>(music_dao);
...
//         music_dao_events::emit_voting_event(
//             voter_addr,
//             music_dao,
//             proposal_id,
//             vote,
...
//     public entry fun resolve(proposal_id: u64, music_dao: address) acquires Proposals, DAO, ProposalVotingStatistics {
//         assert!(exists<DAO>(music_dao), error::not_found(EDAO_NOT_EXIST));
//         let dao = borrow_global<DAO>(music_dao);
...
//         resolve_internal(option::none(), proposal_id, music_dao);
//     }

//     /////////////////////////// Admin flow //////////////////////////////////

//     /// Admin can veto an active proposal
//     public entry fun admin_veto_proposal(admin: &signer, proposal_id: u64, music_dao: address, reason: String) acquires DAO, Proposals {
//         assert!(exists<DAO>(music_dao), error::not_found(EDAO_NOT_EXIST));
//         let dao = borrow_global_mut<DAO>(music_dao);
...
//         let proposals = borrow_global_mut<Proposals>(music_dao);
//         assert!(table::contains(&proposals.proposals, proposal_id), error::not_found(EPROPOSAL_NOT_FOUND));
...
//         music_dao_events::emit_admin_veto_event(
//             proposal_id,
...
//     public entry fun admin_resolve(admin: &signer, proposal_id: u64, music_dao: address, reason: String) acquires DAO, Proposals, ProposalVotingStatistics {
...
//         // assert the proposal is unresolved yet
//         assert!(proposal.resolution == PROPOSAL_PENDING, error::invalid_argument(EPROPOSAL_RESOLVED));
//         resolve_internal(option::some(resolver), proposal_id, music_dao);

//         music_dao_events::emit_admin_resolve_event(
//             proposal_id,
//             signer::address_of(admin),
//             music_dao,
//             reason,
...
//         music_dao_events::emit_admin_offer_event(admin_addr, new_admin, dao);
//     }
...
//         music_dao_events::emit_admin_offer_cancel_event(admin_addr, dao);
//     }
...
//         music_dao_events::emit_admin_claim_event(old_admin, new_admin, dao);
//     }
...
//         music_dao_events::emit_change_name_event(old_name, new_name, dao);
//     }
...
//         music_dao_events::emit_change_threshold_event(old_threshold, new_threshold, dao);
//     }
...
//         music_dao_events::emit_change_duration_event(old_duration, new_duration, dao);
//     }
...
//         music_dao_events::emit_change_voting_power_event(old_power, new_power, dao);
//     }
...
//     /// offer one Music token from DAO to the DST address. The DST address should
...
//     /// Resolve an proposal
//     fun resolve_internal(resolver: Option<address>, proposal_id: u64, music_dao: address) acquires DAO, Proposals, ProposalVotingStatistics {
...
//         assert!(exists<ProposalVotingStatistics>(music_dao), error::not_found(EVOTING_STATISTICS_NOT_FOUND));
//         let proposal_stat = &mut borrow_global_mut<ProposalVotingStatistics>(music_dao).proposals;
...
//         music_dao_events::emit_resolve_event(
//             proposal_id,
//             music_dao,
//             proposal.resolution,
//         )
...
//         // create an Music token collection
...
//         create_proposal(
//             creator,
//             res_acc, // resource account address of the music dao
...
//         create_proposal(
//             admin,
//             dao, // resource account address of the music dao
//             string::utf8(b"Proposal 1"),
//             string::utf8(b"description"),
//             vector<String>[string::utf8(b"no_op")],
//             vector<vector<String>>[vector<String>[]],
